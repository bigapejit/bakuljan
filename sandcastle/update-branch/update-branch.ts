...
resolveConflictsWithAgent().catch((error) => {
  console.error(error);
  process.exit(1);
});
...
async function resolveConflictsWithAgent(): Promise<void> {
  ...
  if (postSha === preMergeSha) {
    fail("Agent produced no commits; branch is still at pre-merge HEAD.");
  }
...
function tryMerge():
  ...
  try {
    ...
    if (conflicts.length === 0) {
      fail("git merge failed but no conflicts were reported; aborting.");
    }
