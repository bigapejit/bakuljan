...
Read `CONTEXT.md` and any relevant ADRs under `docs/adr/` if you need domain context for a comment. Do not go deeper than the comments demand.
...
- `review_thread` - inline thread anchored to a file and line. Only unresolved threads are included. Each comment has a `commentId` you can reply to in-thread.
- `review_summary` - top-level body of a submitted review.

Not everything in here is necessarily actionable. Reviewers may leave context, questions, asides, or things they meant to resolve. Use your judgement. Do not treat unresolved as mandatory action.
...
2. Make the code changes you decided on. Run `npm run typecheck` and `npm run lint` before committing. Use conventional commit messages (`feat:`, `fix:`, `refactor:`, etc.). Do not use a `RALPH:` prefix.
3. If you made no changes, that is fine. Only commit when there is a real diff.
